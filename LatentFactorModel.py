...
class BiasLatentFactorModel(object):
    ...
    def build(self):
        ...
        trainReader = self.data_reader
        train_data = trainReader.get_score_data().values
        bg = batch_generator(train_data, self.batch_size, shuffle=True)
        
        num_prods = trainReader.get_technology_num()
        num_users = trainReader.get_user_num()
        num_batches = int(np.ceil(train_data.shape[0] / self.batch_size))
    ...
    '''
    def test(self):
        testReader = DataReader(self.batch_size, False)
        num_batches = testReader.get_num_batches()
        total_sse = 0
        total_accuracy = 0
        for _ in range(num_batches):
            sse_batch, accuracy_batch = self.sess.run([self.sse, self.accuracy], feed_dict=self.get_feed_dict(testReader.get_next_batch()))
            total_sse += sse_batch
...
if __name__ == '__main__':
    ...
    email_clicked = [model.data_reader.get_clicked_tech_ids(uid) for uid in uids]
    output = pd.concat([pd.DataFrame({'user_id': uids}), df, pd.DataFrame({'email_clicked': email_clicked})], axis=1)
    output.to_csv('LF_result.csv', index=False)
